...
- [ ] `dSpeichern`-Handler **nicht in seiner Lese-Logik ändern** (`dOwner.value`/`dAufgabe.value`/`dFrist.value` bleiben die Wahrheit, werden weiterhin 1:1 in `f.owner`/`f.aufgabe`/`f.frist` übernommen). **Korrektur (Plan-Review — ursprüngliche Ist-Behauptung war falsch):** `dSpeichern` ruft am Ende **`renderAll();dismissDetail();`** auf (Zeilen 5925-5926) — schließt den Drawer also vollständig und ruft **nicht** erneut `openDetail()` auf. Ein Refresh von `dAufgabeView`/Avatar/Ampel *innerhalb* des gerade geschlossenen Drawers ist deshalb unnötig; der nächste `openDetail()`-Aufruf (nächstes Öffnen desselben oder eines anderen Falls) rendert `aufgabenHeroHtml(f)` ohnehin frisch (siehe vorheriger Schritt). Kein zusätzlicher Code-Schritt hier nötig — nur beim Testen daran denken, dass der Drawer nach „Speichern" zu ist, nicht offen mit aktualisiertem Inhalt.
...
- [ ] **Konsistenz mit dem neuen Aufgaben-Hero (Plan-Review-Fund):** Drei Stellen legen neue Fälle mit Status „Neu" an und hardcoden dabei einen alten, freien Aufgaben-Text statt `STATUS_AUFGABE["Neu"]` (aus Task 1.2) zu verwenden — ohne diesen Fix zeigt der neue Aufgaben-Hero (Task 1.3) bei frisch angelegten Fällen kein passendes Icon und einen inkonsistenten Text. `grep -n 'aufgabe:"Erstreaktion' index.html` vor Bearbeitung ausführen, um alle drei Stellen zu bestätigen (Zeilen verschieben sich durch vorherige Tasks). Je 1-Zeilen-Fix:
  - `uebernehmen(id)` ([index.html:4598-4608](../../../index.html#L4598), Stand Planerstellung Zeile 4604): `aufgabe:"Erstreaktion + Daten vervollständigen"` → `aufgabe:STATUS_AUFGABE["Neu"]`.
  - Auto-Simulationspfad in `simulateInbound()` ([index.html:4538-4568](../../../index.html#L4538), Stand Planerstellung Zeile 4556): `aufgabe:"Erstreaktion"` → `aufgabe:STATUS_AUFGABE["Neu"]`.
  - **Zusätzlich selbst gefunden (nicht in der Review-Notiz, dieselbe Baustelle):** `#formNeu`-Submit-Handler — das manuelle „+ Neue Anfrage"-Formular ([index.html:5933-5946](../../../index.html#L5933), Stand Planerstellung Zeile 5945): `aufgabe:"Erstreaktion",frist:document.getElementById("fFrist").value||dstr(1),` → nur den `aufgabe`-Wert ersetzen: `aufgabe:STATUS_AUFGABE["Neu"],frist:document.getElementById("fFrist").value||dstr(1),` (Frist-Teil unverändert).
...
**Ist (verifiziert):** 5 `.chap`-Blöcke (Übersicht/Werdegang/Medizinische Kurzfelder/Abrechnung/Dokumente), jeder rendert reine `.pa-row`-Label:Wert-Paare. Kopf ([index.html:3914-3918](../../../index.html#L3914)) zeigt nur `faName`/`faSub`/`faAva` als Text, kein Achse-Pill, keine Sterne. `faDokumente` ([index.html:6414-6416](../../../index.html#L6414)) nutzt bereits `.status-pill ok/lock` — die Spec-Anforderung „Dokumente als Status-Chips" ist hier **bereits erfüllt**, nur die Positionierung (rechte Spalte) ändert sich. **Bug (Plan-Review, echt):** Im statischen Markup ([index.html:3920-3939](../../../index.html#L3920)) existieren **nur** die Container `faUebersicht`/`faWerdegang`/`faMedizin`/`faAbrechnung`/`faDokumente` — die IDs `faKacheln`/`faNaechste`, in die das JS-Snippet unten schreibt, gibt es nicht. Ohne einen vorgeschalteten Markup-Schritt wirft `renderFallakte()` beim ersten `document.getElementById("faKacheln").innerHTML=...` einen TypeError (`null` hat kein `innerHTML`) und die komplette Fallakte bleibt leer/bricht ab. Deshalb zuerst der Markup-Schritt, dann erst der JS-Schritt.
...
- [ ] **Fehlende Container anlegen (Bug-Fix, MUSS vor dem JS-Schritt passieren):** Im statischen `#view-fallakte`-Markup direkt nach `</header>` und vor dem ersten bestehenden `.chap` („Übersicht") zwei neue, full-width Container einfügen — sie sitzen bewusst **außerhalb** jeder `.chap`/Spalte, damit sie im zweispaltigen Layout (nächster Schritt) oberhalb beider Spalten über die volle Breite laufen:
```html
<!-- Ist: <header class="vhead">...</header><div class="chap"><h2 class="chap-h2">Übersicht</h2><div id="faUebersicht"></div></div> -->
<header class="vhead">...</header>
<div class="fa-kacheln" id="faKacheln"></div>
<div id="faNaechste"></div>
<div class="chap">
...
- [ ] Ab 1024px zweispaltiges Layout: **`#faKacheln`/`#faNaechste` bleiben außerhalb des Grids, laufen volle Breite oberhalb beider Spalten** (siehe Markup-Schritt oben). Darunter: linke Spalte = bestehender „Werdegang"-`.chap` (`faWerdegang`, unverändert) + Verlauf; rechte Spalte = restliche Stammdaten (Kostenträger/Einwilligung/SalutoCare-Zeile aus `faUebersicht`, „Medizinische Kurzfelder" bleibt eigener Block, „Abrechnung"-Chap `faAbrechnung` bleibt ebenfalls eigener, unveränderter Block — die kompakte Kostenzusage-Kette in der Kachel ist ein Kurz-Überblick, `faAbrechnung` bleibt die vollständige Ansicht inkl. Kostenstatus/DRG-Status, bewusste Redundanz, kein Bug) + `faDokumente` (Status-Chips, nur hierher verschoben, Inhalt/Logik unverändert). CSS-technisch per Grid/Flex auf dem `#view-fallakte`-Container ab `min-width:1024px`, unterhalb weiter einspaltig gestapelt.
...
**Sichtprüfung:** Die Fallakte öffnet **ohne Console-Error** (kein TypeError durch fehlende Container) und zeigt oben Achse+Sterne im Kopf, direkt darunter drei Kennzahl-Kacheln (Frist-Ampel-Farbe, Kostenzusage-Kette, Sterne) und eine Nächste-Aufgabe-Karte mit Icon — ab Desktop-Breite stehen Werdegang links und Stammdaten+Dokumente-Chips rechts nebeneinander; nirgends mehr eine reine Fließ-Textliste als Hauptinhalt.
...
- [ ] Nach obigen Änderungen erneut `grep -n "\.zstat" index.html` ausführen (Plan-Review-Korrektur: vollständige Ist-Liste selbst nachgezählt — **neun** Zeilen über sechs Fundstellen verteilt: 395-398/873/883/1775/1779/2637, nicht nur die ursprünglich vier genannten Fundstellen 395-398/1775/1779/2637) — sind nur noch CSS-Definitionen (keine Emission mehr) übrig, sind folgende Regeln jetzt verwaist und können entfernt werden: `.zstat`-Basisregel + `.zstat.aktiv`/`.aufbau`/`.ziel` (Stand Planerstellung [index.html:395-398](../../../index.html#L395)), Override-Block (Stand Planerstellung [index.html:1775](../../../index.html#L1775)/[1779](../../../index.html#L1779)), zweiter Override-Block (Stand Planerstellung [index.html:2637](../../../index.html#L2637)), **dritter Override-Block `.zstat{font-size:12px;letter-spacing:.07em}` (Stand Planerstellung [index.html:873](../../../index.html#L873))**. Ebenso `.zcard.ziel` (Stand Planerstellung [index.html:394](../../../index.html#L394)/[2635](../../../index.html#L2635)) — **`.zcard` selbst (ohne `.ziel`) bleibt**, wird weiterhin für jede Karte gebraucht. **Sonderfall Zeile 883** (Stand Planerstellung, Mehrfach-Selektor-Regel `.t-kicker,.map-title,.zstat{line-height:1.3}`): hier **nicht die ganze Zeile löschen** — `.t-kicker`/`.map-title` sind eigenständige, weiterhin aktiv genutzte Klassen (`.map-title` z. B. in der Netzwerk-Landkarte, [index.html:5193](../../../index.html#L5193)); nur `,.zstat` aus der Selektor-Liste entfernen, Ergebnis `.t-kicker,.map-title{line-height:1.3}`. Vor dem Löschen jeweils per `grep` bestätigen, dass wirklich keine Emission mehr existiert — nicht blind nach Zeilennummer löschen, da sich Zeilen durch vorherige Tasks verschoben haben.
...
**Lane:** `claude-implementer-pro`
**Dateien/Anker:** `mtAbschliessen()` [index.html:6248-6275](../../../index.html#L6248), generischer Zweig [index.html:6266-6270](../../../index.html#L6266), zusätzlich `mtFallAufgabeItem()`/`mtTyp()` [index.html:6032-6047](../../../index.html#L6032) (Plan-Review-Fund, Key-Kollision). **Abhängigkeit:** braucht `advanceFallStatus(f)` aus Task 1.2 — dieser Task darf erst NACH Phase 1 laufen. `grep -n "function mtAbschliessen\|function mtFallAufgabeItem" index.html` vor Bearbeitung erneut ausführen.
...
**BUG (Plan-Review, echt) — `mtTyp()`-Schlüsselkollision bricht genau diese Ketten-Logik:** `mtFallAufgabeItem(f)` ([index.html:6040-6047](../../../index.html#L6040)) baut den `_mtDone`-Key aus `mtTyp(f.aufgabe)` ([index.html:6032-6037](../../../index.html#L6032), bisher weder in Spec noch Plan erwähnt — selbst gefunden vom Plan-Review). `mtTyp()` sortiert den Aufgaben-Text per Regex in nur 4 Buckets (`rueckruf`/`angebot`/`unterlagen`, Default `allgemein`). Von den 5 `STATUS_AUFGABE`-Werten aus Task 1.2 matchen nur zwei einen eigenen Bucket („Rückruf/Erstkontakt"→`rueckruf`, „Unterlagen anfordern"→`unterlagen`); die restlichen drei — „Bedarf qualifizieren", „Kostenzusage anfragen", „Anreise & Zimmer organisieren" — fallen **alle** auf `allgemein` und erzeugen für denselben Fall über mehrere Lebenszyklus-Stufen hinweg denselben Key `"fall:"+f.id+":allgemein"`. Konkrete Folge: hat ein Fall z. B. die Stufe „Bedarf qualifizieren" bereits im Sheet abgeschlossen (Key `"fall:8:allgemein"` in `_mtDone`), zeigt `mtSheetRender()` bei der späteren Stufe „Kostenzusage anfragen" (derselbe Key!) fälschlich die „✓ Erledigt"-Bestätigung statt des Arbeits-Buttons — genau die in diesem Task gebaute Ketten-Logik (`naechstes=mtFallAufgabeItem(f)`, siehe unten) würde damit auf ein bereits-erledigt-Item zeigen, das in Wahrheit noch offen ist.

**Selbst verifiziert — `mtTyp()` hat im gesamten Code genau eine Aufrufstelle** (Zeile 6042, innerhalb `mtFallAufgabeItem`). Eine Regex-Erweiterung (`/qualifizier/i`→neuer Bucket, `/anreise|zimmer/i`→neuer Bucket) hätte deshalb keine Auswirkung auf andere Konsumenten — birgt aber ein eigenes Kollisionsrisiko: der naheliegende Name `"kosten"` für „Kostenzusage anfragen" ist **bereits vergeben** durch das komplett unabhängige `typ:"kosten"` aus `mtKostenItem()` ([index.html:6060-6073](../../../index.html#L6060), eigener Key `"fall:"+f.id+":kosten"`, eigener Entstehungsweg über `f.status`/`f.kosten`, nicht über `f.aufgabe`-Text) — zwei inhaltlich verschiedene Items hätten dann denselben Key, ein neues, schwerer zu findendes Kollisionsproblem.

**Gewählte Variante: Key in `mtFallAufgabeItem()` von typ-basiert auf statusbasiert umstellen, `mtTyp()` selbst unangetastet lassen.** `f.status` ist innerhalb `STATUS` (Task 1.2) garantiert eindeutig — keine Bucket-Namen-Kollision möglich, kein Risiko wie bei der Regex-Variante. `typ` (aus `mtTyp(f.aufgabe)`) bleibt als separates Feld im zurückgegebenen Item unverändert bestehen, da es weiterhin `MT_TYP_LABEL`/`MT_SCHRITTE`/`MT_LEITFAEDEN`/`MT_NOTIZ_LABEL` speist (dortige Generizität über mehrere ähnliche Aufgaben hinweg ist gewollt und bleibt — nur die Abschluss-Eindeutigkeit war das Problem, nicht die Content-Bucketing).

**Schritte:**
- [ ] `grep -n "function mtFallAufgabeItem" index.html` — Zeile neu bestätigen. Im `if(f.aufgabe){...}`-Zweig **nur den Key** umstellen, `typ` als Feld unverändert lassen:
```js
// Ist: return {key:"fall:"+f.id+":"+typ,typ:typ,kind:"fall",ref:f,name:f.name,
//  kontext:f.quelle||f.kanal,step:f.aufgabe,
//  block:(d!==null&&d<=0)?"jetzt":"woche",overdue:(d!==null&&d<0)};
return {key:"fall:"+f.id+":"+f.status,typ:typ,kind:"fall",ref:f,name:f.name,
 kontext:f.quelle||f.kanal,step:f.aufgabe,
 block:(d!==null&&d<=0)?"jetzt":"woche",overdue:(d!==null&&d<0)};
```
  Der Rückfall-Loop darunter (`const TYPES=[...]`, sucht `_mtDone` nach alten typ-basierten Keys für Fälle mit leerem `f.aufgabe`) bleibt unangetastet — nicht Teil dieses Bugs. **Anmerkung, kein Handlungsbedarf:** selbst per `grep -n '\.aufgabe *= *""' index.html` gegengeprüft — nach Task 1.2/diesem Fix gibt es im gesamten Code keine Stelle mehr, die `f.aufgabe` bei nicht-terminalem Status leert; der Rückfall-Loop wird zwar weiterhin erreicht (z. B. Seed-Fall id 13 „Werner Aumann", `aufgabe:""` bei Status „Unterlagen"), findet aber nie mehr einen Treffer und liefert praktisch immer `null` — als spätere Vereinfachungs-Kandidat notiert, nicht Teil dieser Spec.
...
- [ ] Standard-Verifikation, erweitert um **3-Hop-Test (Plan-Review-Vorgabe, deckt die Schlüsselkollision ab)**: einen S.-Koordination-Fall im Status „Neu" wählen (z. B. per Konsole `faelle[0].status="Neu";faelle[0].aufgabe=STATUS_AUFGABE["Neu"];faelle[0].frist=dstr(0);renderAll();` falls kein Seed-Fall passt) und im Mitarbeiter-Sheet **dreimal hintereinander** abschließen: Neu→Kontaktiert (Hop 1), Kontaktiert→Qualifizierung (Hop 2), Qualifizierung→Unterlagen (Hop 3). Bei **jedem** der drei Hops muss der Arbeits-Button („Erledigt · ins Protokoll" mit leerem, noch auszufüllendem Notizfeld) erscheinen — **nie** die „✓ Erledigt"-Bestätigung für eine Stufe, die gerade erst neu erreicht wurde. Ohne den Key-Fix oben würde spätestens beim dritten Hop („Kostenzusage anfragen", Bucket `allgemein`, kollidiert mit „Bedarf qualifizieren" vom ersten Hop) fälschlich „✓ Erledigt" erscheinen.

**Sichtprüfung:** Nach Abschluss einer Fall-Aufgabe im Mitarbeiter-Sheet erscheint direkt die nächste Aufgabe **desselben Falls** (erkennbar am unveränderten Namen/Avatar im Header, aber neuem Aufgabentext und aktualisiertem Status) — nicht die nächste beliebige Aufgabe aus der allgemeinen Warteschlange. **Den 3-Hop-Test (Neu→Kontaktiert→Qualifizierung→Unterlagen) vollständig durchspielen: bei jedem der drei Abschlüsse erscheint der editierbare Arbeits-Button, kein einziges Mal fälschlich „✓ Erledigt" für eine frisch erreichte Stufe.**
