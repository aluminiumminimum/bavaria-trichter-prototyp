...
| 2498 | `#ovDetail .d-acc,#ovDetail .docs{border-radius:4px}` | **Soll (entschieden):** **beide** Teile referenzieren `#ovDetail` → die Regel wird **nicht ersatzlos gestrichen**, sondern zieht auf den neuen Fallakte-Kontext um: `#view-fallakte .d-acc,#view-fallakte .docs{border-radius:4px}` (die Steuerung (③) verwendet `.d-acc` weiter, `.docs` steht in der Arbeit②, aus `dArbeitHtml()`s Unterlagen-Zweig — beide landen unter `#view-fallakte`, der Selektor bleibt also nach dem Umzug ebenso präzise wie zuvor). Sonst fiele die 4px-Radius-Etiketten-Konvention in der Akte auf die generische 12px-Basisregel von `.d-acc`/`.docs` zurück. |
...
**Zusammenfassung CSS-Waisen:** **8 echte Waisen-Regel-Blöcke** (784–790 zusammen als 1 Block gezählt, 1884, 1886\*, 1888–1889 als 1 Block, 2490–2493 als 1 Block, 2496, 2499) + **1 Regel, die auf den neuen Fallakte-Kontext umzieht statt gelöscht zu werden** (2498, s. o. — festes Soll) + **4 weitere gruppierte Selektoren, die gekürzt statt gelöscht werden** (2216, 2219, 2462, 2465). \*1886 ist zusätzlich redundant mit einer bestehenden globalen Regel, ihr Entfernen hat also ohnehin keinen visuellen Effekt.
...
- **Zuordnungs-Hinweis (`f.zuordnungsHinweis`) als goldene Zeile falls gesetzt:** **Soll (entschieden):** erscheint **nur im Kopf**, nirgends sonst in der Akte. `aufgabenHeroHtml()` rendert denselben Hinweis heute noch zusätzlich als `.ah-hinweis`-Zeile innerhalb des Hero (Arbeit①) — diese Zeile wird aus `aufgabenHeroHtml()` entfernt, um die Dopplung zu vermeiden (einzige verbleibende Konsumentin dieser Funktion ist Arbeit① der Akte — nach Entfernung von `#ovDetail` gibt es keinen zweiten Aufrufer mehr, der die Zeile bräuchte). `mtSheet`s `.mt-shinweis`-Anzeige ([index.html:6662](../../../index.html#L6662)) bleibt unangetastet — anderer Arbeitsplatz (`ma-mode`), eigene Anzeige, keine Dopplung mit dem Kopf der Akte.
...
**② Dynamischer Arbeitsbereich:** **Soll (entschieden):** die ID `dArbeit` wird wörtlich beibehalten (kein Umbenennen in einen `.fk-*`-Namen) — ebenso `dLog`, `dReply`, `dReplyKanal`, `dNotizSofort`, `u1`–`u4`, `docCnt`, `dKZ` (alle Teil von `dArbeitHtml()`s Ausgabe) und die Modul-Variable `aktuellerFall`. Grund: `sendReply()`/`kzNotizAdd()`/`updateDocCnt()`/`dArbeitKostenUpload()` referenzieren diese IDs direkt per `getElementById`, und `kzAnfragen`/`kzZusage`/`kzAblehnung`/`sendReply`/`advanceFall`/der Speichern-Handler lesen/schreiben `aktuellerFall` direkt — ein Umbenennen würde mehrere Funktionskörper anfassen, ohne fachlichen Nutzen (Surgical-Changes-Prinzip). **Nur neu hinzukommende Elemente** (Stage-Band im Kopf, neue Grid-Wrapper der Zwei-Zonen-Struktur, Steuerung-Collapse-Wrapper falls zusätzliches Markup nötig) bekommen `.fk-*`-Klassen — bestehende, umziehende IDs/Klassen bleiben unverändert. `renderFallakte()` ergänzt `document.getElementById("dArbeit").innerHTML=dArbeitHtml(f);` (identischer Aufruf wie bisher in `openDetail()`). Danach funktionieren `kzAnfragen`/`kzZusage`/`kzAblehnung`/`sendReply`/`kzNotizAdd`/`dArbeitKostenUpload`/`dArbeitUnterlagenAnfordern`/`updateDocCnt` unverändert, **sofern** `aktuellerFall` korrekt gesetzt ist (§2.4-Fix) und `kzAnfragen`/`kzZusage`/`kzAblehnung`s Re-Render-Zeile auf `renderFallakte()` statt `openDetail()` umgestellt wird (3 Stellen).
